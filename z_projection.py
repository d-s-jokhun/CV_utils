...
def z_projection (id_str, replacement_str):
    ...
    source_dir = "/MBELab/jokhun/Pro 2 - IntelZoom/Data/40x"
    ...
    filenames = [f for f in listdir(source_dir) if ('.TIF' in f and 'DIC' not in f)]
    source_path = [join(source_dir,f) for f in filenames]
    des_path = [join(des_dir,'40x_'+f[14:]) for f in filenames]  # '100x_'+f[17:], '60x_'+f[16:], '40x_'+f[14:]
